...
// 상품 중복 확인
const productExist = (userID, productID, productName, productPrice, callback) =>{
    console.log("productExist 호출됨.")
    const data = [ userID, productID ]
    const param = {'productID':productID,'productName':productName,'productPrice':productPrice}
    const sql = 'select exists (select userID=?,productID=? from product_list) as isChk'
...
//신규주문 중복 확인
const orderExist = (orders, orderCount, callback) =>{
    console.log("orderExist 호출됨.")
    const data = [ orders.ordNo ]
    const param = {"orders":orders, "orderCount":orderCount}
    const sql = 'select exists (select orderID=? from order_list) as isChk'
    executeSQL(sql,data,param,callback)
}
//신규주문 저장
const saveOrders = (userID, orders, orderCount, callback)=>{
    ...
    const param = {}
    const sql = 'insert into order_list set ?'
    executeSQL(sql,data,param,callback)
}
//신규주문의 갯수
...
module.exports = {productExist, saveProducts , updateStock, updatePrice, getProductID, getProducts,sumProductPrice, 
                  orderExist,saveOrders, getOrdersCount, getOrdersList, getCustomerOrdersList, getUserPin, updateWaybill}